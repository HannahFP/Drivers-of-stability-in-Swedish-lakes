...
y.hat[i]<- mu + l[lake[i]] + ye[year[i]] + size[i]*s + latitude[i]*lat + Rich[i]*richness + TP[i]*TotP + Alk[i]*Alkal + Temp[i]*temperature
}
...
mu.Lake.sigma[j] <- mu2 + sizemean[j]*smean + latitudemean[j]*latmean + richmean[j]*rmean + ALKmean[j]*alkmean + TPmean[j]*tpmean + Tmean[j]*tempmean
}
...
jags1 <- jags.model("C:/Users/hhen0001/Documents/Stability_BMI_56Lakes/Scripts/JAGSmodelbelowDCA1.txt" , data=belowDCA1, n.chains=1, n.adapt=1000)
smp1 <- coda.samples(jags1, c("mu2", "sigma.lake","res.sigma.lake","s", "lat", "TotP", "Alkal","richness", "smean", "latmean", "rmean", "alkmean", "tpmean", "tempmean","temperature","sigma.l", "sigma.y"), n.iter = 3050000, thin=3000, burnin=50000, n.chains=1, inits=inits)
